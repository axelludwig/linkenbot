...
bot.on('message', msg => {
    ...
    if (msg.content === 'test') {
    ...
    } else if (msg.content.includes('deco') || msg.content.includes('bouge') ||
        msg.content.includes('degage') || msg.content.includes('dégage') ||
        (msg.content.includes('pars') && msg.content.includes('bot')) ||
        (msg.content.includes('stop') && msg.content.includes('linkenbot'))
    ) {
        msg.member.voice.channel.leave();
    }
    else if (msg.content.includes('viens') || msg.content.includes('reviens') || (msg.content.includes('bot') && msg.content.includes('venir'))) {
       ...
       ⣿⣿⣿⣶⣶⣮⣥⣒⠲⢮⣝⡿⣿⣿⡆⣿⡿⠃⠄⠄⠄⠄⠄⠄⠄⣠⣴⣿⣿⣿
        ...
        // var t = msg.member.user.tag + 'a dit ' + msg.content;
...
function playSound(bot, msg, fileName, message) {
    ...
    var channel = bot.channels.cache.get(channelId);
    channel.join().then(connection => {
        const dispatcher = connection.play(`sounds\\${fileName}`);
